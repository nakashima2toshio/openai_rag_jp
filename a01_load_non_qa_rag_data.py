...
3. livedoor_news: livedoorニュースコーパス（日本語ニュース）
4. cc_news: CC-News（英語ニュース記事）
5. scientific_papers: Scientific Papers（学術論文）
"""
...
class NonQARAGConfig:
    ...
    DATASET_CONFIGS = {
        ...
        "livedoor_news": {
            ...
            "description"     : "日本語ニュース記事データセット",
            "hf_dataset"      : "shunk031/livedoor-news-corpus",
            "hf_config"       : "default",
            "split"           : "train",
            "streaming"       : True,
            "text_field"      : "content",
        ...
        # Scientific Papers (arXiv & PubMed)
        "scientific_papers": {
            "name"            : "Scientific Papers（arXiv & PubMed）",
            "icon"            : "🔬",
            "required_columns": ["abstract", "article"],
            "description"     : "学術論文（arXiv & PubMed）本文＋要約",
            "hf_dataset"      : "armanc/scientific_papers",
            "hf_config"       : "pubmed",  # pubmed or arxiv
            "split"           : "train",
            "streaming"       : True,
            "text_field"      : "abstract",
            "title_field"     : None,
...
def extract_text_content(df: pd.DataFrame, dataset_type: str) -> pd.DataFrame:
    ...
    # Multi-News用の特別処理（documentフィールドの処理）
    if dataset_type == "multi_news" and 'document' in df.columns:
        df_processed['Combined_Text'] = df_processed['document'].apply(
            lambda x: clean_text(str(x)) if x is not None else ""
        )
    # タイトルとテキストを結合
    elif title_field and title_field in df.columns and text_field in df.columns:
        # タイトルがある場合は結合
    ...
    # Scientific Papers用の特別処理（要旨と本文を結合）
    if dataset_type == "scientific_papers" and 'article' in df.columns:
        df_processed['Combined_Text'] = df_processed.apply(
            lambda
                row: f"{clean_text(str(row.get('abstract', '')))} {clean_text(str(row.get('article', ''))[:1000])}".strip(),
            # 本文は最初の1000文字のみ
            axis=1
        )
...
def main():
    ...
    with st.sidebar:
        ...
        **選択中のデータセット:**
        ...
        elif selected_dataset in ["livedoor_news", "cc_news"]:
        ...
        elif selected_dataset == "scientific_papers":
            dataset_specific_options['preserve_citations'] = st.checkbox(
                "引用を保持",
                value=True,
                help="論文引用形式を保持"
            )
            dataset_specific_options['include_article'] = st.checkbox(
                "本文を含める",
                value=True,
                help="要旨に加えて本文も含める"
            )
            dataset_specific_options['paper_type'] = st.selectbox(
                "論文タイプ",
                options=["pubmed", "arxiv"],
                help="PubMed（医学）またはarXiv（数理科学）"
            )

        elif selected_dataset == "stackoverflow":
            dataset_specific_options['include_tags'] = st.checkbox(
                "タグを含める",
                value=True,
                help="質問のタグ情報を含める"
            )
            dataset_specific_options['min_score'] = st.number_input(
                "最小スコア",
                min_value=0,
                value=0,
                help="この点数以上の質問のみ（データセットに含まれる場合）"
            )
            ...
            if st.button("📥 HuggingFaceからロード", type="primary"):
                try:
                    ...
                    with st.spinner(f"HuggingFaceから{dataset_name}をダウンロード中..."):
                        ...
                        elif dataset_name == "shunk031/livedoor-news-corpus":
                            # livedoorニュースコーパス
                            st.info(f"📥 {dataset_name}をロード中...")
                            dataset = hf_load_dataset(dataset_name, config_name if config_name else "default", split=split_name, streaming=True)

                        elif dataset_name == "armanc/scientific_papers":
                            # Scientific Papers
                            st.info(f"📥 {dataset_name}をロード中 (config: {config_name or 'pubmed'})...")
                            dataset = hf_load_dataset(dataset_name, config_name if config_name else "pubmed", split=split_name, streaming=True)
    ...
    with tab2:
        ...
        if 'uploaded_data' not in st.session_state:
            ...
            elif selected_dataset in ["japanese_text", "livedoor_news", "cc_news"]:
                specific_issues = validate_news_data_specific(df, selected_dataset)
            elif selected_dataset == "scientific_papers":
                specific_issues = validate_scientific_data_specific(df, selected_dataset)
            else:
                # その他のデータセットの検証
                specific_issues = []
                if 'text' in df.columns:
